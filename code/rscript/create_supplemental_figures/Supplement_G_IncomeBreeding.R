...
if(!"out.path" %in% ls()){
  ...
  out.path <- here("simulations", sim.date,"Supplement_LowFreqHighIntensity_incomebreeding",  "processed", "output_aggregated", "output_stacked", "stacked_aggregated_output.csv")
}
...
##################################
#### INCOME BREEDING FIGURE ######
##################################

# create a subset where fast and slow end of each population is retained

IncomeBreeding_data_sub <- stacked %>%
  group_by(disturbance.interval, disturbance.intensity) %>% 
...
(p1_plot_popdensGenerationTime <- ggplot(IncomeBreeding_data_sub, aes(x = pop_dens/2500 , y = log10(generation_time), color = tot_coefvar, group = sim.id))+
...
(p1_plot_BRN_paper <-
    ggplot(IncomeBreeding_data_sub, aes(y = medianBT, x = medianLH))+
...
# save figure 4
ggsave(here::here("figs", sim.date, "supplemental", paste0("S1_G_IncomeBreeding", ".jpeg")), IncomeBreeding, width = 16, height = 10, units = "cm", dpi = 600)
ggsave(here::here("figs", sim.date, "supplemental", paste0("S1_G_IncomeBreeding", ".pdf")), IncomeBreeding, width = 16, height = 10, units = "cm", dpi = 600)
