...
if(!all(substr(basename(animal.files), 11, 35) == substr(basename(metadata.files), 1, 25))) warning("Metadata and animal files do not match!")
match(substr(basename(metadata.files), 1, 25), substr(basename(animal.files), 11, 35))
...
if(!exists("return_f2_")) return_f2_ = F
if(!exists("start.at_")) start.at_ = 10000

cl <- makeCluster(n_cores)
registerDoSNOW(cl)
foreach(i = seq(length(file_list)), .packages = c("tidyverse", "here")) %dopar%
  process_sim_step2(X = file_list[[i]], out.path_aggregated = out.path_aggregated, out.path_intermediate = out.path_intermediate, start.at = start.at_, return_f2 = return_f2_)
stopCluster(cl)
