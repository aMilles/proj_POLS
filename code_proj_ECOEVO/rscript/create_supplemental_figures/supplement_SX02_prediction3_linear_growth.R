...
### FIT LINEAR MODELS, linear growth type
### with generation time grouped into slow and fast types by the median

model_stacked <- stacked %>% filter(growth_type == "linear")
...
fit_lin <- glm(r0 ~ gt_group + gt_group : pop_dens, model_stacked %>% filter(growth_type == "linear"), family = "gaussian")

# create data.frame with population density, generation time groups and growth type
pred.df <- expand.grid(gt_group = unique(model_stacked$gt_group), pop_dens = seq(min(model_stacked$pop_dens), max(model_stacked$pop_dens), length.out = 100), growth_type = c("linear"))
...
### comparison P3 between logistic and linear growth
...
# save figure
ggsave(here::here("figs", sim.date, "supplemental", paste0("Fig_SX02", ".jpeg")), Fig_SX02, width = 8, height = 8, units = "cm", dpi = 600)
